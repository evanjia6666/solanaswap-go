...
func NewCreateAmmConfigInstruction(
	...
	// Add the accounts to the instruction.
	{
		// Account 0 "owner": Writable, Signer, Required, Address: DmwXqqK5Zuj619au6q2Jx3TMr9ZV1837uxJcEwyvXVtV
...
// Builds a "create_permissionless_farm_switch" instruction.
// Creates a permissionless farm switch account for the program //  // # Arguments //  // * `ctx`- The context of accounts //
func NewCreatePermissionlessFarmSwitchInstruction(
	ownerAccount solanago.PublicKey,
	permissionlessFarmSwitchAccount solanago.PublicKey,
	systemProgramAccount solanago.PublicKey,
) (solanago.Instruction, error) {
	accounts__ := solanago.AccountMetaSlice{}
	...
	// Add the accounts to the instruction.
	{
		// Account 0 "owner": Writable, Signer, Required, Address: DmwXqqK5Zuj619au6q2Jx3TMr9ZV1837uxJcEwyvXVtV
		accounts__.Append(solanago.NewAccountMeta(ownerAccount, true, true))
		// Account 1 "permissionless_farm_switch": Writable, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(permissionlessFarmSwitchAccount, true, false))
		// Account 2 "system_program": Read-only, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(systemProgramAccount, false, false))
	}
...
func NewInitializeRewardInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		// Account 5 "permissionless_farm_switch": Read-only, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(permissionlessFarmSwitchAccount, false, false))
...
// Builds a "toggle_permissionless_farm_switch" instruction.
// Toggle the permissionless farm switch account //  // # Arguments //  // * `ctx`- The context of accounts // * `is_on`- Is the permissionless farm allowed or not //
func NewTogglePermissionlessFarmSwitchInstruction(
	...
	// Encode the instruction discriminator.
	err := enc__.WriteBytes(Instruction_TogglePermissionlessFarmSwitch[:], false)
	if err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}
	{
		// Serialize `isOnParam`:
		err = enc__.Encode(isOnParam)
		if err != nil {
			return nil, errors.NewField("isOnParam", err)
		}
	}
	accounts__ := solanago.AccountMetaSlice{}
...
func NewTransferRewardOwnerInstruction(
	...
	// Add the accounts to the instruction.
	{
		// Account 0 "authority": Read-only, Signer, Required, Address: DmwXqqK5Zuj619au6q2Jx3TMr9ZV1837uxJcEwyvXVtV
		// Address to be set as operation account owner.
		accounts__.Append(solanago.NewAccountMeta(authorityAccount, false, true))
		// Account 1 "pool_state": Writable, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(poolStateAccount, true, false))
...
func NewUpdateAmmConfigInstruction(
	...
	// Add the accounts to the instruction.
	{
		// Account 0 "owner": Read-only, Signer, Required, Address: DmwXqqK5Zuj619au6q2Jx3TMr9ZV1837uxJcEwyvXVtV
