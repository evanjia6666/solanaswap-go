...
func TestAnomalyXsc9(t *testing.T) {
	data, err := os.ReadFile("/tmp/xsc9_abnormal_tx_wrapped.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, item := range transactionData {
		...
		if tx.InputMintDecimals > 0 && tx.OutputMintDecimals > 0 {
			inputDec := float64(tx.InputAmount) / float64(pow10(tx.InputMintDecimals))
			outputDec := float64(tx.OutputAmount) / float64(pow10(tx.OutputMintDecimals))
			if inputDec > 0 {
				price := outputDec / inputDec
				fmt.Printf("    Implied price: %.8f\n", price)
			}
		}
		fmt.Println()
	}
